...
WITH from_ods AS (
    ...
    separate_date_time AS(
    SELECT
        o.id_trip,
        CAST(o.pickup_datetime AS DATE) as pickup_date,
        CAST(o.dropoff_datetime AS DATE) as dropoff_date,
        ...
        o.dropoff_datetime as dropoff_datetime,
        o.passenger_count,
        o.trip_distance,
        o.fare_amount,
        o.extra,
    ...
    with_pickup_hour AS (
        ...
        o.store_and_fwd_flag,
        o.payment_type_fk,
        EXTRACT(HOUR FROM o.pickup_time)::SMALLINT AS pickup_hour,
        EXTRACT(HOUR FROM o.dropoff_time)::SMALLINT AS dropoff_hour,
    FROM separate_date_time o
    ),
...
CASE
    WHEN o.pickup_hour BETWEEN 5 AND 11 THEN 'Morning Rush'
    WHEN o.pickup_hour BETWEEN 12 AND 15 THEN 'Midday'
    WHEN o.pickup_hour BETWEEN 16 AND 19 THEN 'Evening Rush'
    WHEN o.pickup_hour BETWEEN 20 AND 23 THEN 'Night'
    ELSE 'Late Night'
END AS time_of_day_category

FROM with_pickup_hour as o
...
LEFT JOIN date_pickup_lookup AS dp ON o.pickup_date = dp.date
LEFT JOIN date_dropoff_lookup AS dd ON o.dropoff_date = dd.date
LEFT JOIN weather_lookup AS wl ON zp.borough_name = wl.borough_name
        AND o.pickup_date = wl.weather_date
        AND o.pickup_hour = HOUR(wl.weather_time)
